...
resource "aws_instance" "keycloak" {
  ...
  user_data                   = <<-EOL
    ...
    docker run --restart=always -p 80:8080 -e KEYCLOAK_ADMIN=${local.keycloak_admin_username} -e KEYCLOAK_ADMIN_PASSWORD=${random_password.keycloak.result} -e KC_PROXY=passthrough -d quay.io/keycloak/keycloak:21.0.2 start-dev
  EOL
...
resource "aws_ssm_parameter" "keycloak_admin_credentials" {
  ...
  type  = "StringList"
  value = "${local.keycloak_admin_username},${random_password.keycloak.result}"
}
